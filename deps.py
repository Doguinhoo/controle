...
def liberate(path, type):
    ...
    while True:
        ...
        if verify_exit in ['', 's', 'y', 'sim', 'yes']:
            ...
            df.loc[result_df.index[0], 'Saida'] = time.strftime('%d/%m/%Y %H:%M:%S')
...
def add_row_csv(path, row: dict):
    ...
    print(f"*                                    Pessoas dentro: {df['Saida'].isnull().sum()}")
...
def cadastro(path, type):
    while True:
        ...
        while not cadastro['Confirmado']:
            ...
            telefone = input(f'Telefone do {type}: ')
            if  telefone != '':
                while not validate_telefone(cadastro['Telefone']):
                    print('Telefone inválido')
                    telefone = input(f'Reenvie telefone do {type}: ')
            ...
            if cadastro['Confirmado']:
                ...
                # add_unique_row_csv(path, cadastro)#  depecrated
                add_row_csv(path, cadastro)
                print("Registro adicionado com sucesso.")
